...
def save_req(lfile, str):
    f = open(lfile, "a")
    """It is modified on the next line, line 90, as to support milisecond
...
#------------- Add resolved IP address to record with -------------------------
			str_ans=resp.answer[0].to_text()
			ls_ans=str_ans.split()
			# add resolved IP to print out logfile
			save_req(LOGREQFILE, 'Client IP: ' + address[0] + '    request is    ' + str(q) + '  '+ ls_ans[4] + '\n')
#------------------------------------------------------------------------------
                else:
                    # not implemented
                    resp = make_response(qry=msg, RCODE=4)  # RCODE =  4    Not Implemented
...
def std_AAAA_qry(msg):
    ...
    for host in hosts:
	#---------------------DNS Block Added here---------------------------------------- 
	#if host is not in nospoof list
        if (iparpa[:-1] not in nospoof):
	    #if host in in spoof list, then add fake address
            if iparpa[:-1] in spoof:
		#print("in spoof:%s\n")
                #save_req(LOGREQFILE, '!!! Specific host (' + iparpa + ') asked....\n')
                print("spoof IPV6 host:%s\n" %(iparpa))
                for spoof_ip in spoof[iparpa].split(","):
                    DEBUGLOG('Adding fake IP6 = ' + spoof_ip)
                    rrset = dns.rrset.from_text(iparpa, 1000, dns.rdataclass.IN, dns.rdatatype.AAAA, spoof_ip)
                    resp.answer.append(rrset)
                return resp
	#-------------------------------------------------------------------------------------------------------
        DEBUGLOG('Adding ' + host.to_text())
